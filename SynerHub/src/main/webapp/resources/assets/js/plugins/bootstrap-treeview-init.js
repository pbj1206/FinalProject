$(document).on("click", "#thButton2, #projectPayment", function () {
  var chAndTh;
  ...
  $.ajax({
    url: "/synerhub/document/getChAndTh",
    type: "post",
    async: false,
  ...
  for (let j = 0; j < chAndTh.thList.length; j++) {
    let data = {
      text: chAndTh.thList[j].thTtl,
      href: "#child" + j,
      tags: ["0"],
      thNo: chAndTh.thList[j].thNo
    }

    defaultData[0].nodes.push(data);
  }
  ...
  var treeview1 = $(document).find("#treeview1");
  treeview1.treeview({
  ...
  treeview1.on("click", "li", async function () {
    let data = {
      chNo: synerhubch,
      thNo: synerhubth,
      memNo: MEM_NO
    ...
    var options = $(document).find("select[id=thMemSelect] option");
    options.remove();

    res.data.forEach(mem => {
      console.log(mem);
      if($(this).text() == mem.chTtl){
        $("select[id=thMemSelect]").append(`<option value="${mem.memNo}">
          ${mem.chRoleNm}
        &emsp;
          ${mem.chMemNm}
        &emsp;
      ...
      }else if ($(this).text() == mem.chMemThNm) {
        $("select[id=thMemSelect]").append(`<option value="${mem.memNo}">
              ${mem.chRoleNm}
            &emsp;
              ${mem.chMemNm}
